...
library(ggmap);library(raster);library(leaflet);library(maps);library(maptools);
...
```{r UserInput}
# Where should perpindicular transects begin along the baseline transect (km; 0.003 = 3 m)
perp.spacing.start <- 0.003
# What should the spacing of the transects be (km; 0.006 = 6 m)
perp.spacing <- 0.006
...
for(i in 1:nrow(sites)){
  ...
  tx.interval <- seq(perp.spacing.start,sites$base_distance[i],perp.spacing)
